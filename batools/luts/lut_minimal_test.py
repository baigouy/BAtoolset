...
import math
from batools.img import int24_to_RGB
import matplotlib.pyplot as plt
import numpy as np
from matplotlib.backends.backend_agg import FigureCanvasAgg
from matplotlib.figure import Figure
from batools.img import fig_to_numpy
import base64
...
def serialize_lut(lut_256_3):
    ...
    # this generates a string that can be serialized
    encoded_data = base64.b64encode(lut_256_3).decode()
    return encoded_data

def deserialize_lut(string_lut, make_editable=True):
    # see also /home/aigouy/mon_prog/Python/epyseg_pkg/personal/FigGen_EzFig2/trash/tst_base64_encode_the_whole_np_array.py
    decoded_data = np.frombuffer(base64.b64decode(string_lut), dtype=np.uint8)
    decoded_data = decoded_data.reshape((256, 3))
    if make_editable:
        decoded_data = decoded_data.copy()  # required to make it editable
    return decoded_data
...
def apply_lut(img, lut, convert_to_RGB=False, min=None, max=None, do_not_interpolate=False):
    """
...
def convert_ome_lut(original_lut):
    """
    Convert a 3-element 16-bit LUT to a (256, 3) 8-bit LUT.

    Parameters:
    original_lut (list or np.array): A 3-element list or array representing R, G, B values in 16-bit range.

    Returns:
    np.array: A (256, 3) numpy array representing the 8-bit LUT.
    """
    # Ensure input is a numpy array and of correct type
    original_lut = np.array(original_lut, dtype=np.uint16)

    # Check if input has correct shape
    if original_lut.shape != (3,):
        raise ValueError("Input LUT must have exactly 3 elements")

    # Scale down to 8-bit range (0-255)
    scaled_lut = (original_lut / 65535 * 255).astype(np.uint8)

    # Create a (256, 3) array
    full_lut = np.zeros((256, 3), dtype=np.uint8)

    # Fill each channel with a gradient
    for i in range(3):
        full_lut[:, i] = np.linspace(0, scaled_lut[i], 256, dtype=np.uint8)
