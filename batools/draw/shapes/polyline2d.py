...
class PolyLine2D(Polygon2D):
    ...
    def __init__(self, *args, coords_as_list=None,color=0xFFFFFF, opacity=1., stroke=0.65, line_style=None, theta=0, fill_color=None, __version__=1.0, **kwargs):
    ...
    def draw(self, painter, draw=True, parent=None):
        ...
        if draw:
            ...
            if parent is not None and parent.scale is not None and parent.scale != 1:
                polyline_to_draw = self.__get_scaled_polyline(polyline_to_draw,1. / parent.scale)

            if parent is not None:
                polyline_to_draw = polyline_to_draw.translated(parent.topLeft())


            # print(polyline_to_draw)
    ...
    def listVertices(self, polygon=None):
        if polygon is None:
            polygon = self
        return [point for point in polygon]

    def __get_scaled_polyline(self, polygon, scale):
    ...
    # def __get_scaled_polyline(self, scale):
