from batools.draw.shapes.ellipse2d import *
...
class Circle2D(Ellipse2D):
    ...
    def __init__(self, *args,x=0, y=0, width=0, color=0xFFFFFF, fill_color=None, opacity=1., stroke=0.65,line_style=None, __version__=1.0, **kwargs):
        # print('args here',args)
        if len(args) == 3:
            super(Circle2D, self).__init__(*args, args[-1])
        elif len(args) == 4:
            logger.error("too many values, cirle only has, x,y and width")
        else:
            if args:
                super(Circle2D, self).__init__(*args)  # create empty circle
            else:
                super(Circle2D, self).__init__(x,y,width, width)  # create empty circle
    ...
    def add(self, *args): # this is required for drawing a cicrle otherwise it becomes as ellipse...
        p1 = args[0]
        p2 = args[1]
