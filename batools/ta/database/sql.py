...
import batools.SQLite_tools.tools
from batools.img import Img, has_metadata, _create_dir
from batools.ta.selections.selection import convert_coords_to_IDs
from batools.tools.early_stopper_class import early_stop
from batools.tools.logger import TA_logger  # logging
from batools.utils.loadlist import smart_TA_list,loadlist
...
def get_table_columns(path_to_db, table_name):
    ...
    """
    return batools.SQLite_tools.tools.get_table_columns(path_to_db, table_name)
...
def query_db_and_get_results(db_path, SQL_command_to_run):
    ...
    from batools.ta.database.sql import TAsql  # Import the required module
...
def table_exists_in_db(db_path, table_name):
    ...
    #     True
    """
    from batools.ta.database.sql import TAsql
    db = None
    exists = False
    try:
        db = TAsql(db_path)
...
# TODO --> maybe also allow to save the master db --> pb is that it is never created the request is just made on the fly
