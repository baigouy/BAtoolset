...
# from batools.figure.fig_tools import preview
...
class Panel(Rect2D):
    ...
    def createTable_neo(self):
        ...
        # from batools.figure.row import Column
        ...
        for i in range(self.nRows):
            ...
            if row is not None and not len(row) == 0:
                from batools.draw.widgets.image_organizer import packX
                # packing seems correct then need align top
                packX(row, self.space) # ça ça marche
                from batools.draw.widgets.image_organizer import alignTop
        ...
        from batools.draw.widgets.image_organizer import packY
        # not necessarily that by the way
        # packY(fig, space=self.space)
        from batools.draw.widgets.image_organizer import sameHeight
        ...
        from batools.draw.widgets.image_group import group
        if self.width() != 0:
    ...
    def createTable(self):
        ...
        # why the hell do I have a figure in panel really need to recode that
        from batools.draw.widgets.col import col
        fig = col(space=self.space)
        self.rows=[]
...
if __name__ == '__main__':
    from batools.draw.shapes.image2d import Image2D  # KEEP Really required to avoid circular imports
